package cj.netos.fission.service;

import cj.netos.fission.IFissionAccountService;
import cj.netos.fission.IInAbsorbActivityController;
import cj.netos.fission.model.AbsorbInRecord;
import cj.studio.ecm.annotation.CjService;
import cj.studio.ecm.annotation.CjServiceRef;
import cj.studio.ecm.net.CircuitException;

@CjService(name = "inAbsorbActivityController")
public class InAbsorbActivityController implements IInAbsorbActivityController {
    @CjServiceRef(refByName = "records.fissionRecordService")
    IFissionRecordService fissionRecordService;
    @CjServiceRef
    IFissionAccountService fissionAccountService;

    @Override
    public void inAbsorb(AbsorbInRecord record) {
        try {
            fissionAccountService.inAbsorb(record);
            fissionRecordService.inAbsorbSucceed(record);
        } catch (Exception e) {
            CircuitException ce = CircuitException.search(e);
            if (ce == null) {
                ce = new CircuitException("500", e);
            }
            int status = Integer.valueOf(ce.getStatus());
            String message = ce.getMessage();
            if (message.length() > 40) {
                message = message.substring(0, 40);
            }
            message.replace("\n", "");
            fissionRecordService.inAbsorbError(record, status, message);
            throw e;
        }
    }

    @Override
    public void error(AbsorbInRecord record, int status, String msg) {
        fissionRecordService.inAbsorbError(record, status, msg);
    }
}
